...
// 2. cargo new entity --lib
// https://github.com/SeaQL/sea-orm/blob/master/examples/axum_example/src/main.rs

mod flash;

use axum::{
	Router, Server, Json, Error,
...
async fn main () -> anyhow::Result<()> {
	...
	let addr = SocketAddr::from_str(&server_uri).unwrap();
...
async fn create_post (
	...
	post::ActiveModel {
		kanji_name: Set(model.kanji_name.to_owned()),
		eng_name: Set(model.eng_name.to_owned()),
		age: Set(model.age.to_owned()),
		bloodtype: Set(model.bloodtype.to_owned()),
		birthplace: Set(model.birthplace.to_owned()),
		..Default::default()
	}
	.save(conn)
	.await
	.expect("could not insert port");
